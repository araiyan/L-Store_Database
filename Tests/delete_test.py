...
for key in records:
    ...
    if (randint(0, 20) < 4):
        ...
        new_records[key] = records[key].copy()
...
for key in records:
    ...
    # here we are sure that there is only one record in t hat array
    # check for retreiving version -1. Should retreive version 0 since only one version exists.
    record = query.table.index.locate(query.table.key, key)
    print(record)

    if key in new_records:
        if not record:
            print('select error on', key, ':', record, ', correct:', new_records[key])

    elif record:
            print('select error on', key, ':', record, ', correct:', None)